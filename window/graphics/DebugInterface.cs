...
public class DebugInterface {
    ...
    public void HandleDeviceLost(Device device) {
        var removedReason = device.DXDevice.DeviceRemovedReason;

        Debug.WriteLine($"Device removed! Code: {removedReason.Code.ToString()} Description: {removedReason.Description}");

        using var dred = device.DXDevice.QueryInterfaceOrNull<ID3D12DeviceRemovedExtendedData1>();
