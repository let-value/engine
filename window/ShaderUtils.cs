...
public static class ShaderUtils {
    public static ReadOnlyMemory<byte> CompileBytecode(DxcShaderStage stage, string shaderName, string entryPoint) {
        var shaderSource = File.ReadAllText(Path.Combine(AppContext.BaseDirectory, shaderName));

        using var includeHandler = new ShaderIncludeHandler(AppContext.BaseDirectory);
